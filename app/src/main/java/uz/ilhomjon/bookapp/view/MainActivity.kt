...
class MainActivity : AppCompatActivity() {
    ...
    private fun loadData() {
        ...
        myBookViewModel.getCategoryNames(this).observe(this){
            when (it.status) {
                MyStatus.LOADING ->{
                    Log.d(TAG, "loadData: loading")
                ...
                MyStatus.SUCCESS ->{
                    ...
                    categoriesRVAdapter = CategoriesRVAdapter(l!!, object :CategoriesRVAdapter.RvClick{
                        override fun onClick(title: String) {
                            myBookViewModel.getBookByCategoryList(this@MainActivity, title).observe(this@MainActivity){
                                when(it.status){
                                    MyStatus.LOADING->{
                                    ...
                                    MyStatus.SUCCESS->{
                                        ...
                                        val categoryBooksAdapter = CategoryBooksAdapter(it.data?.results?.books!!)
                                        binding.trendingBooksRV.adapter = categoryBooksAdapter
                                    }
                                    MyStatus.ERROR->{
                                        binding.progressTrendingBooks.visibility = View.GONE
                                        Toast.makeText(this@MainActivity, "Error category", Toast.LENGTH_SHORT)
                                            .show()
                                    }
                                }
                            }
